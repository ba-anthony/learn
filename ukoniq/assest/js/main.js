...
(function ($) {
    ...
    jQuery(document).ready(function ($) {
        ...
        if(window.location.hash === "#requestconsultant"){
          $(window.location.hash).modal('show');
        }
        if(window.location.hash === "#starterkit"){
            $(window.location.hash).modal('show');
          }
...
$(document).ready(function(e){
    ...
    $(".table_row h3").click(function(e){
        if($(window).width() < 992){
            $(this).toggleClass("active");
            $(this).parents(".table_row").find(".table_row_info").slideToggle();
        }
    })
    $(".column_head").click(function(e){
        if($(window).width() < 992){
            $(this).toggleClass("active");
            $(this).parents("li").find(".table_body").slideToggle();
        }
    })
});
...
window.addEventListener("orientationchange", function() {
   // $("#fullchart").modal('hide');
}, false);

 //vimeo video cover image
 $(document).ready(function(e){
 var iframe = document.querySelector('#vimvid');
 var player = new Vimeo.Player(iframe);
 
 player.on('play', function () {
   $(".banner").addClass("removebanner");
   $(".bg_image").addClass("removemzlfl");
 });
});
