...
for(var a = 0; a < tabs.length; a++) {
  ...
  tabs[a].onclick = function() {
    ...
    for(var b = 0;b < tabs.length; b++) {
      tabs[b].style.backgroundColor = unseletectedColor;
      tabs[b].style["font-weight"] = "normal";
    }
...
var previous, next, serviceTitle, serviceText, currentIdex, previousIdex;
...
next.onclick = function() {
  ...
  if(currentIdex == (ourServices.length - 1)) {
    currentIdex = 0;
  } else {
    currentIdex += 1;
  }

  var title = ourServices[currentIdex].title;
  var text = ourServices[currentIdex].text;
...
previous.onclick = function() {
  ...
  if(currentIdex == 0) {
    currentIdex = ourServices.length - 1;
  } else {
    currentIdex -= 1;
  }
